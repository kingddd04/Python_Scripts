...
def file_size(dirname) -> float:
    tot_size = 0
    all_path_names = os.listdir(dirname)
    for path in all_path_names:
...
def search_word(filename: str, word: str) -> int:
    ...
    for row in rows:
        low_row = row.lower()
        temp_count = low_row.count(lower_word)
        loc_word_count += temp_count
    return loc_word_count
