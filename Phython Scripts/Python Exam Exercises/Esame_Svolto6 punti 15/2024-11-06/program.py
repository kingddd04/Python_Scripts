...
def func1(int_list, keys):
    res = {}
    keys_l = list(keys)
    for numero in keys_l:
        res[numero] = []
        for numero_1 in int_list:
            if numero_1 % numero == 0:
                res[numero].append(numero_1)
                res[numero].sort(reverse=True)
    return res
...
def func5(input_pngfile):
    Matrix = images.load(input_pngfile)
    counter = 0
...
def Conta_Pari_Disp(root, risultato):
    if root:
        if root.value % 2 == 0:
            risultato[0] += 1
        else:
            risultato[1] += 1
        Conta_Pari_Disp(root.left, risultato)
        Conta_Pari_Disp(root.right, risultato)
        return risultato
    
    
def ex_1_Helper(root):
    if root is None:
        return 0
    
    Somma_Destr = ex_1_Helper(root.right)
    Somma_Sinist = ex_1_Helper(root.left)
    root.value += Somma_Destr + Somma_Sinist
