...
"""
Certo! L'operatore &= è un operatore di assegnazione che combina l'intersezione 
tra set con l'aggiornamento del set stesso. In altre parole, 
common_elements &= set(lst) è un'abbreviazione di common_elements = common_elements 
& set(lst), dove & è l'operatore di intersezione.
"""
#Funziona +2
def func1(lists):
    # Trova l'intersezione di tutte le liste interne
    common_elements = set(lists[0])
    for lst in lists[1:]:
        common_elements &= set(lst)
    
    # Ordina gli elementi comuni dal più grande al più piccolo
    return sorted(common_elements, reverse=True)

# Esempio di utilizzo
lists = [[4, 4, 10, 4, 1], [4, 2, 1], [1, 4]]
print(func1(lists))  # Output: [4, 1]
...
def func3(textfile_in, textfile_out):
    ...
    molt_par = 1
    molt_dis = 1
    with open(textfile_in, "r", encoding="utf8") as YOO:
        lines = YOO.readlines()
    for line in lines  :
        clean_l = line.replace(",","")
        numbers = clean_l.split()
        int_l = [int(x) for x in numbers]
        for n in int_l:
            if n % 2  == 0:
                molt_par *= n 
                somma_pari += n
                ...
                molt_dis *= n 
                somma_dispari +=n            
        diff_par_disp = molt_par-molt_dis
        res.append(str(diff_par_disp)+"\n")
        molt_par = 1
        molt_dis = 1
        
    inv_res = res[::-1]
    with open(textfile_out, "w", encoding="utf8") as finale:
        finale.writelines(inv_res)
...
output {(150, 200, 150): 3, (200, 150, 125): 4, (125, 175, 200): 2,
        (125, 100, 125): 3, (125, 175, 125): 4, (200, 100, 150): 2}
'''
...
def func5(png_input):
    res = {}
    Matrix = images.load(png_input)
    for y in range(1,len(Matrix)-1):
        for x in range(1, len(Matrix[0])-1):
            if Matrix[y][x] != (0,0,0):
                new_col = Matrix[y][x]
                if Matrix[y+1][x+1] == new_col and  Matrix[y-1][x-1] == new_col and  Matrix[y+1][x-1] == new_col and Matrix[y-1][x-1] == new_col:
                    if new_col not in res:
                        res[new_col] = 0
                    res[new_col] += 1
    return res
...
def ex2_H1(dirin, dizionario, extensions):
    ...
    for path in os.listdir(dirin):
        ...
        if os.path.isfile(Fullpath):
            ...
            if splitted_path[1] in extensions:
                int_siz = os.path.getsize(Fullpath)
                if int_siz > max_int:
                    max_int = int_siz 
                if int_siz < min_int:
                    min_int = int_siz
    if max_int != 0:
        dizionario[dirin] = [min_int,max_int]
    for path in os.listdir(dirin):
        Fullpath = dirin +"/"+ path
        if os.path.isdir(Fullpath):
            ex2_H1(Fullpath,dizionario, extensions)
    return dizionario
            

def ex2(dirin, extensions):
    diz_out = ex2_H1(dirin, {} , extensions)
    return diz_out
