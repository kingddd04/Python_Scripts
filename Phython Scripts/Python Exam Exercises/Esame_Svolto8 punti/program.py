...
def ex1_H11(dirin, res, profondita):
    for path in os.listdir(dirin):
        fullpath = dirin +"/"+ path
        if fullpath.endswith(".txt"):
            int_N = ex1_H(fullpath)
        res.append((fullpath, res, profondita))
            
            
def ex1(dirin):
    tuple_L = ex1_H11(dirin, [], 0)
    return tuple_L
...
def ex2(root):
    res = [0]  # Using a list to allow modification within the helper function
    ex2_H(root, res)
    return res[0]
