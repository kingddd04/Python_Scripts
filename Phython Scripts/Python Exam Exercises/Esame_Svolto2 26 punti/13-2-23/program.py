...
def func1(D):
    ...
    for chiave in D:
        ...
        res.append(lista[chiave])
    return res
        
#print(func1({4: ["c", "h", "f", "g", "e"], 2: ["a", "z", "b", "w"], 0: ["a", "b", "a"]}))
#print(func1({1: ["c", "h", "f", "g", "e"], 0: ["a", "z", "b", "w"], 2: ["a", "b", "a"]}))
...
def ex1_H(root, depht, out_res):
    ...
    if depht > len(out_res):
        out_res.append(0)
    loc_V = root.V
    out_res[depht] += loc_V
    ex1_H(root.left, depht+1, out_res)
    ex1_H(root.right, depht+1, out_res)
    return out_res
...
def ex2_H2(fullpath, word):
    occ_n = 0
    with open(fullpath, "r", encoding="utf8") as FIN :
        text = FIN.read()
    words_txt = text.split()
    for sush in words_txt:
        if sush == word:
            occ_n += 1
    return occ_n
            
    
def ex2_H1(dirin,words,res):
    for path in os.listdir(dirin):
        fullpath = dirin+"/"+path
        if fullpath.endswith(".txt"):
            for word in words :
                Occ_Numb = ex2_H2(fullpath, word)
                if word not in res:
                    res[word] = 0
                res[word] += Occ_Numb
    for path in os.listdir(dirin):
...
def ex2(dirin, words):
    tuple_l = []
    dic_out = ex2_H1(dirin, words, {})
    for chiave in dic_out:
        out_T = (chiave, dic_out[chiave])
        tuple_l.append(out_T)
    tuple_l.sort(key = lambda x: (-x[1],x[0]))
    return tuple_l

#print(ex2("C:\\Users\\david\\OneDrive\\Documents\\Phython Scripts\\Esercizi Per L'Esame\\esame-13-2-23\\13-2-23\\ex2", ["gatto", "cane"]))
